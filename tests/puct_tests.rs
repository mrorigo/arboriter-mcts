use arboriter_mcts::{
    ...
    policy::{expansion::ExpansionPolicy, selection::PUCTPolicy},
    tree::MCTSNode,
...
fn test_expansion_policy_sets_prior() {
    ...
    let mut mcts = MCTS::new(state, config).with_expansion_policy(expansion_policy);
    ...
    assert_eq!(
        root.children[0].prior(),
        0.8,
        "Prior should be set by policy"
    );
